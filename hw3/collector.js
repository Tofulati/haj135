(function () {
  ...
  const config = {
    endpoint: 'https://collector.yourdomain.com/collect',
    enableVitals: true,
    enableErrors: true,
    sampleRate: 1.0,
    debug: false,
  ...
  function getNetworkInfo() {
    if (!('connection' in navigator)) return { effectiveType: 'unknown' };
  ...
  function getTechnographics(imageSupported) {
    return {
      userAgent: navigator.userAgent,
      language: navigator.language,
      cookiesEnabled: navigator.cookieEnabled,
      javascriptEnabled: true,
      imagesEnabled: imageSupported !== undefined ? imageSupported : null,
      cssEnabled: checkCSSSupport(),
  ...
  function getNavigationTiming() {
    ...
    return {
      ...
      totalLoadTime: round(n.loadEventEnd - n.startTime),
      ...
      timingObject: n.toJSON ? n.toJSON() : {}
    };
  }
  ...
  function collect(type, imageSupported) {
    let payload = merge(buildBasePayload(type || 'pageview'), {
      title: document.title,
      referrer: document.referrer,
      enterTime: PAGE_ENTER_TIME,
      technographics: getTechnographics(imageSupported),
  ...
  function reportError(errorData) {
    ...
    errorCount++;
    send(merge(buildBasePayload('error'), { error: errorData }));
  ...
  function resetIdleTimer() {
    if (idleStart !== null) {
      ...
      if (duration >= IDLE_THRESHOLD) {
        send(merge(buildBasePayload('idle'), {
          idleEnd: Date.now(),
  ...
  function initActivityTracking() {
    document.addEventListener('mousemove', (e) => {
      resetIdleTimer();
      send(merge(buildBasePayload('mouse'), { event: 'mousemove', x: e.clientX, y: e.clientY }));
    });

    document.addEventListener('click', (e) => {
      resetIdleTimer();
      send(merge(buildBasePayload('mouse'), { event: 'click', button: e.button, x: e.clientX, y: e.clientY }));
    });

    document.addEventListener('scroll', () => {
      resetIdleTimer();
      send(merge(buildBasePayload('scroll'), { x: window.scrollX, y: window.scrollY }));
    });

    document.addEventListener('keydown', (e) => {
      resetIdleTimer();
      send(merge(buildBasePayload('keyboard'), { event: 'keydown', key: e.key, code: e.code }));
    });

    document.addEventListener('keyup', (e) => {
      resetIdleTimer();
      send(merge(buildBasePayload('keyboard'), { event: 'keyup', key: e.key, code: e.code }));
    });
  ...
  function initTimeOnPage() {
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden') {
        ...
        const exitPayload = merge(buildBasePayload('page_exit'), {
          enterTime: PAGE_ENTER_TIME,
          leaveTime: Date.now(),
  ...
  const publicAPI = {
    init: function (options) {
      ...
      const run = () => {
        send(merge(buildBasePayload('session_enter'), { enterTime: PAGE_ENTER_TIME }));

        const img = new Image();
        img.onload = () => setTimeout(() => collect('pageview', true), 0);
        img.onerror = () => setTimeout(() => collect('pageview', false), 0);
    ...
    track: function (eventName, eventData) {
      ...
      const payload = merge(buildBasePayload('event'), {
        event: eventName,
        data: eventData || {},
        customData: customData
      });
  ...
  window.addEventListener('beforeunload', () => {
    if (!initialized || blocked) return;
    send(merge(buildBasePayload('session_leave'), {
      enterTime: PAGE_ENTER_TIME,
      leaveTime: Date.now(),
      duration: Date.now() - PAGE_ENTER_TIME
    }));
  });
